 ...
 *         2.如果要解决代码中出现的异常,需要添加非常复杂的代码逻辑来进行判断,会使代码变得非常臃肿,不利于维护,可读性
 *           比较差，因此推荐大家使用异常机制来处理程序运行过程中出现的问题
 *         3.程序在运行过程中如果出现了问题,会导致后面的代码无法运行,而使用异常机制之后,可以对异常情况进行处理
 ...
 *              try{代码逻辑} catch (具体的异常) {异常处理逻辑} catch (具体的异常) {}
 *                  可以针对每一种具体的异常做相应的更丰富的处理  注意:当使用多重的catch的时候一定要注意相关异常的顺序,将子类
 *                  放在最前面的catch,父类放在后面的catch
 *       执行过程中可能存在的情况:
 *          1.正常执行,只执行try中的代码
...
public class TestException {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            System.out.println("出现数学异常");
            e.printStackTrace();
        } catch (InputMismatchException e) {
            System.out.println("输入的参数类型不匹配");
            e.printStackTrace();
        }
